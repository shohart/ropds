...
/// `update_author_key`, `count_doubles`, and `get_duplicate_groups` work on MySQL.
/// This covers the `CONCAT()` function used for MySQL COUNT(DISTINCT ...) queries.
#[tokio::test]
async fn mysql_author_key_and_duplicate_detection() {
    let (_container, pool) = start_mysql().await;
    ...
    // hide_doubles with COUNT(DISTINCT CONCAT(...)) on MySQL
    assert_eq!(
...
/// Transactional `set_book_authors_and_update_key` works on MySQL.
#[tokio::test]
async fn mysql_set_book_authors_and_update_key() {
    let (_container, pool) = start_mysql().await;
